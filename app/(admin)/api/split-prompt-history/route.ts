...
import { splitPromptHistory } from '@/lib/db/schema';
...
export async function GET(request: NextRequest) {
  ...
  const modelCodeName = searchParams.get('modelCodeName') || 'all';
  ...
  try {
    ...
    const historyQuery = db
      ...
      .from(splitPromptHistory)
      .where(modelCodeName !== 'all' ? eq(splitPromptHistory.modelCodeName, modelCodeName) : undefined)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { prompt, modelCodeName } = await request.json();
    if (!prompt || !modelCodeName) {
      return NextResponse.json({ error: 'Missing prompt or modelCodeName' }, { status: 400 });
    }
