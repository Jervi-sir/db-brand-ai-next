// File: app/api/content/voice-over/reschedule/route.ts
...
export async function POST(request: Request) {
  ...
  try {
    // Validate that the contentId belongs to the user
    ...
    console.log('Existing content check:', {
      ...
      found: existingContent.length > 0,
    });
    ...
    // Parse and normalize the new deadline
    const deadline = new Date(newDeadline);
    deadline.setHours(23, 0, 0, 0); // Set to 23:00:00

    console.log(`Rescheduling content ${contentId}:`, {
    ...
    console.log('Updated content:', {
      ...
      deadline: updatedContent.deadline?.toISOString(),
    });
